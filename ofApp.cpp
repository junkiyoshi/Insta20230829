...
void ofApp::update() {
	...
	for (auto& noise_seed : this->noise_seed_list) {
		...
		for (int i = 0; i < 3; i++) {
			...
			this->size_list.push_back(ofRandom(5, 10));

			color.setHsb((int)ofRandom(180 + index * 128, 255 + index * 128) % 255, 200, 255);
	...
	for (int i = this->location_list.size() - 1; i > -1; i--) {
		...
		this->size_list[i] *= 1.01;

		if (this->color_list[i].getBrightness() > 0) { this->color_list[i].setBrightness(this->color_list[i].getBrightness() - 3); }
